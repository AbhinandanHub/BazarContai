...
//start: it is for Insert (means add) category
if(isset($_POST['add_category_btn']))
{
    $name= $_POST['name'];  //here the $name come from add-category.php page (where name="name") ;
    $slug= $_POST['slug'];
    $description= $_POST['description'];
    $meta_title= $_POST['meta_title'];
    $meta_description= $_POST['meta_description'];
    $meta_keywords= $_POST['meta_keywords'];
    $status= isset($_POST['status']) ? '1':'0'; //here we use tarnary operator
    $popular= isset($_POST['popular']) ? '1':'0';

    $image = $_FILES['image']['name'];    // here ['image'] - this image comes from name=image in add-category.php page ; and ['name'] - means name of the selected image
    $path = "../uploads";

    $image_ext = pathinfo($image, PATHINFO_EXTENSION);  //$image_ext means image extension
    $filename = time().'.'.$image_ext; // this is concardinate method

    //insert the data (image) in our database
    ...
    if($cate_query_run)
    {
        move_uploaded_file($_FILES['image']['tmp_name'],$path.'/'.$filename);   //"$path.'/'.$filename" this is concartinate
...
//End: it is for Insert (means add) category


//start: it is for update (means in edit section) category
...
//End: it is for update (means in edit section) category

//start: it is for delete the category
...
//End: it is for delete the category

//start: it is for add-product.php 
else if(isset($_POST['add_product_btn']))
{
    ...
    if($name != "" && $slug != "" && $description != "" )  //this line is for validation part (not null any input field)
    {
...
//End: it is for add-product.php 

//Start: it is for update (means in edit section) in edit-product.php
else if(isset($_POST['update_product_btn']))
{
    ...
    if($new_image != "")
    {
        // $update_filename = $new_image;
        $image_ext = pathinfo($new_image, PATHINFO_EXTENSION); 
        $update_filename = time().'.'.$image_ext; //concartinate the existing image into new image
    }
    else
    {
        $update_filename= $old_image;
   ...
   if($update_product_query_run)
   {
       if($_FILES['image']['name'] != "")
       {
           move_uploaded_file($_FILES['image']['tmp_name'], $path.'/'.$update_filename); //addm the image into 'uploads' folder
           if(file_exists("../uploads/".$old_image))
           {
               unlink("../uploads/".$old_image);
           }
...
//End: it is for update (means in edit section) in edit-product.php

//Stert: How to confirm and delete product through ajax callng (custom.js)
else if(isset($_POST['delete_product_btn']))
{
    ...
    if($delete_query_run)
    {
        ...
        echo 200; //means success
    }
    else
    {
        // redirect("products.php", "Something went wrong");
        echo 500;  //means error
    }
}
