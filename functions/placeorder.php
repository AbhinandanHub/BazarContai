...
if(isset($_SESSION['auth']))
{
    if(isset($_POST['placeOrderBtn']))
    {
        ...
        $tracking_no= "contaibazar".rand(1111,9999).substr($phone,2); 
        ...
        if($insert_query_run)
        {
            ...
            foreach ($query_run as $citem)
            {
                ...
                //if anyoue buy some product then reduce the quantity of products from out stock (in database)
                $product_query = "SELECT * FROM products WHERE id='$prod_id' LIMIT 1 "; //selecting one product so LIMIT 1
